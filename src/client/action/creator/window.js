...
export default (registry, api, store) => {
  ...
  function deferredSelect(id, mapStore) {
    const timeout = delay(DEFERRED_SELECT_TIMEOUT).then(() => Promise.reject("Giving up on selecting item after timeout"));
    ...
    const select = new Promise((resolve) => {
      const subscription = mapStore.addListener(() => {
        ...
        if (mapStore.has(id)) {
          ...
          debug(
            "Got change event in deferred select but store still does not have item"
          );
        }
      });
    }).then((subscription) => {
