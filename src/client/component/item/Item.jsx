...
export default class Item extends Component {
  ...
  render() {
    const click = registry
      .getHandler(types.itemSelect)
      .bind(undefined, this.props.id);
    ...
    return (
      ...
      >
        <h4>
          {this.props.selected ? (
            <span className="playing-indicator fa "></span>
          ) : null}
          {this.props.title}
        </h4>
        <div
          className="item-thumbnail"
          style={{ backgroundImage: 'url("' + this.props.thumbnail.url + '")' }}
        >
          <img src={this.props.thumbnail.url} alt={this.props.title} />
        </div>
      </article>
    );
  }
