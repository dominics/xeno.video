...
export default class MenuUserSettings extends Component {
  ...
  render() {
    ...
    if (!this.props.setting.authenticated) {
      ...
      options.push(
        <li key="login-separator" role="separator" className="divider" />
      );
    }
    ...
    options.push(
      <Checkbox
        key="autoplay"
        id="autoplay"
        checked={this.props.setting.autoplay}
        title="Autoplay"
      />
    );
    options.push(
      <Checkbox
        key="nsfw"
        id="nsfw"
        checked={this.props.setting.nsfw}
        title="Show NSFW"
      />
    );
    ...
    if (!FavouriteChannelStore.isDefault(this.props.favouriteChannel)) {
      options.push(
        <li key="reset-separator" role="separator" className="divider" />
      );
      options.push(
        <li key="reset-button" className="reset-button">
          <a
            onClick={registry
              .getHandler(types.channelReset, true)
              .bind(undefined, null)}
          >
            Reset Channels
          </a>
    ...
    if (this.props.setting.authenticated) {
      options.push(
        <li key="logout-separator" role="separator" className="divider" />
      );
      options.push(
        <li key="logout">
    ...
    return <ul className="dropdown-menu dropdown-menu-right">{options}</ul>;
  }
}
