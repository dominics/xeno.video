...
export default (configInstance) => {
  ...
  deps.factory("queue.itemByChannel", (container) => container.factory("item:by-channel"));

  deps.factory("queue.channelsForUser", (container) => container.factory("channel:for-user"));
  ...
  deps.service("router", () => {
    ...
    deps
      .digest(["route.index", "route.user", "route.api", "route.error"])
      .forEach((route) => {
        route(router);
      });
